			...
			ini.parse(path.join(file.basedir, file.filename), function(err, data) {
				...
				try {
					if (data['Desktop Entry']['Exec']) {
						...
						// Ignore application which shouldn't be executed on our desktop environment
						if (data['Desktop Entry'].hasOwnProperty('OnlyShowIn')) {
							var desktops = data['Desktop Entry']['OnlyShowIn'].split(';');
							var ignore = true;

							for (var i in desktops) {
								if (desktops[i] == 'OPENBOX' || desktops[i] == 'JUICE') {
									ignore = false;
